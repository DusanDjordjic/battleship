...
#define UNREACHABLE(file, line) { fprintf(stderr, "UNREACHABLE %s %d\n", __FILE__, __LINE__);  exit(1); }
...
error_code do_signup(client_state_t* state);
error_code do_login(client_state_t* state);
error_code do_logout(client_state_t* state);
                ...
                switch (choice) {
                    ...
                    case 0:
                        ...
                        if (err == ERR_NONE) {
                            ...
                            fprintf(stderr, "we failed to logout and reached unrecoverable state, closing...\n");
                            goto EXIT;
                        }
                        break;
                }
...
error_code do_login(client_state_t* state) {
    ...
    switch (err) {
        ...
        case ERR_IO_IIN:
            ...
            return err;
        case ERR_IO_IARG:
            // unreachable because we pased USERNAME_MAX_LEN as string len
            UNREACHABLE(__FILE__, __LINE__);
        case ERR_IO_UNKNOWN:
